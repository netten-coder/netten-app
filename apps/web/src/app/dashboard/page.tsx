...
function StatCard({ label, value, sub, icon, href }: { label: string; value: string; sub?: string; icon: React.ReactNode; href?: string }) {
  const inner = (
    <div className={`card flex items-start gap-4 ${href ? 'cursor-pointer hover:border-brand/40 transition-colors' : ''}`}>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('netten_token') : null
    if (!token) {
  ...
  if (error || !data) {
    return (
      <div className="p-8 flex items-center justify-center min-h-64 flex-col gap-4">
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/dashboard/payments" className="text-brand text-sm hover:text-brand-light transition-colors">View all →</Link>
        </div>
