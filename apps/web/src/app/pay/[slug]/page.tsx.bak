    ...
    <div className="min-h-screen bg-brand-dark flex flex-col items-center justify-start px-4 py-8 md:py-16">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-6">
          ...
          {link.amountUsd ? (
            <p className="text-white font-bold text-4xl mt-1">${link.amountUsd.toFixed(2)}</p>
          ) : (
            <p className="text-gray-300 text-lg mt-1">{link.description}</p>
          )}
          {link.amountUsd && (
        ...
        {step === 'select' && (
          <div className="space-y-3">
            ...
            {/* Custom amount if link has no fixed price */}
            {!link.amountUsd && (
              <div className="mb-2">
        ...
        {step === 'qr' && txn && (
          <div className="text-center">
            <p className="text-gray-400 text-sm mb-6">
              Send exactly <span className="text-white font-medium">{txn.payAmount} {txn.fromCoin}</span> to this address
            </p>

            {/* QR */}
            <div className="inline-block p-4 bg-white rounded-2xl mb-4">
              <QRCodeSVG
                value={txn.payAddress}
                size={200}
                bgColor="white"
                fgColor="#041E17"
                level="M"
            ...
            <p className="text-gray-500 text-xs font-mono break-all bg-surface-card rounded-xl px-3 py-2 mb-4">
              {txn.payAddress}
            </p>

            <button
              onClick={() => navigator.clipboard.writeText(txn.payAddress)}
        ...
        {step === 'waiting' && (
          <div className="text-center py-6">
            ...
            <p className="text-gray-600 text-xs mt-6">Please keep this page open</p>
          </div>
        )}
        ...
        {step === 'done' && (
          <div className="text-center py-6">
            ...
            <p className="text-gray-400 text-sm">Your payment has been received and settled in RLUSD.</p>
            <p className="text-gray-500 text-xs mt-4">Powered by Netten · XRPL</p>
          </div>
        )}
