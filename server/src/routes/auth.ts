...
export async function authRoutes(app: FastifyInstance) {
  app.post('/login', async (req, reply) => {
    ...
    if (!onWaitlist && !isFounder) return reply.status(403).send({ error: 'Not on waitlist', message: 'Netten is in early access. Join the waitlist at netten.app' })
    let merchant = await db.merchant.findUnique({ where: { email } })
    if (!merchant) merchant = await (db.merchant as any).create({ data: { email } })
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:40px 20px;"><h2 style="color:#041E17;margin-bottom:8px;">Sign in to Netten</h2><p style="color:#64748B;margin-bottom:24px;">Click the button below to sign in. This link expires in 10 minutes.</p><a href="${loginUrl}" style="display:inline-block;background:#1D9E75;color:white;padding:14px 28px;border-radius:10px;text-decoration:none;font-weight:600;">Sign in to Netten →</a><p style="color:#94A3B8;font-size:12px;margin-top:24px;">If you didn't request this, ignore this email.</p></div>`,
    })
  ...
  app.get('/verify', async (req, reply) => {
    ...
    const magicToken = await db.magicToken.findUnique({ where: { token }, include: { merchant: true } })
    ...
    // FIX: sameSite:'none' required because frontend (netten.app) and API (railway.app) are different domains
    // sameSite:'strict' was preventing the cookie from ever being sent cross-origin
    reply.setCookie('netten_refresh', refreshRaw, { httpOnly: true, secure: true, sameSite: 'none', maxAge: 7 * 24 * 60 * 60 })
